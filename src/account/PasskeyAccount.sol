...
contract PasskeyAccount is SimpleAccount, IPasskeyAccount {
     ...
     */
    function initialize(string calldata credId, uint256 pubKeyX, uint256 pubKeyY) external initializer {
        super._initialize(address(0));
        _initPasskey(credId, pubKeyX, pubKeyY);
    }

    function updatePasskey(string calldata credId, uint256 pubKeyX, uint256 pubKeyY) external virtual override {
        require(msg.sender == address(this), "Only wallet can update passkeys");
        require(pubKeyX != 0 && pubKeyY != 0, "Zero passkey is not allowed");
        require(passkey.pubKeyX != 0 && passkey.pubKeyY != 0, "Passkey doesn't exist");
    ...
    function _initPasskey(string memory credId, uint256 pubKeyX, uint256 pubKeyY) internal {
        require(passkey.pubKeyX == 0 && passkey.pubKeyY == 0, "Passkey already exists");
        passkey.credIdHash = keccak256(abi.encodePacked(credId));
        passkey.pubKeyX = pubKeyX;
        passkey.pubKeyY = pubKeyY;
    ...
    function _validateSignature(
        ...
        (
            ...
            uint256 r,
            uint256 s
        ) = abi.decode(userOp.signature, (bytes, bool, string, uint256, uint256, uint256, uint256));
        ...
        returns (bool)
    {
        ...
        (bool success, bytes memory ret) = address(p256Verifier).staticcall(args);
    ...
    function _verifySignatureWebauthn(
        ...
        if (authenticatorData.length < 37 || !_checkAuthFlags(authenticatorData[32], requireUserVerification)) {
            return false;
        }
    ...
    function _contains(string memory substr, string memory str, uint256 location) internal pure returns (bool) {
    ...
    function _checkAuthFlags(bytes1 flags, bool requireUserVerification) internal pure returns (bool) {
