...
contract PasskeyAccount is SimpleAccount, IPasskeyAccount {
    ...
    Passkey private currentPasskey;
    Passkey public genesisPasskey;
    ...
    function updatePasskey(
        ...
        require(
            _publicKeyX != 0 && _publicKeyY != 0,
            "Zero passkey is not allowed"
        );
        require(
            currentPasskey.pubKeyX != 0 && currentPasskey.pubKeyY != 0,
            "Passkey doesn't exist"
        );

        currentPasskey.pubKeyX = _publicKeyX;
        currentPasskey.pubKeyY = _publicKeyY;
        emit PasskeyUpdated(currentPasskey.pubKeyX, currentPasskey.pubKeyY);
    }
    ...
    function _setPasskey(uint256 _publicKeyX, uint256 _publicKeyY) internal {
        require(
            currentPasskey.pubKeyX == 0 && currentPasskey.pubKeyY == 0,
        ...
        currentPasskey.pubKeyX = _publicKeyX;
        currentPasskey.pubKeyY = _publicKeyY;
        genesisPasskey.pubKeyX = _publicKeyX;
        genesisPasskey.pubKeyY = _publicKeyY;
        emit PasskeySet(currentPasskey.pubKeyX, currentPasskey.pubKeyY);
    }
    ...
    function _validateSignature(
        ...
        return
            _verifySignatureWebauthn(
                ...
                r,
                s,
                currentPasskey.pubKeyX,
                currentPasskey.pubKeyY
            )
                ? 0
    ...
    function _verifySignatureAllowMalleability(
        ...
        (bool success, bytes memory ret) = address(p256Verifier).staticcall(
            args
        );
